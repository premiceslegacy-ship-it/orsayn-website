...
<div class="container">
  ...
  <div id="panel-csv" class="panel active">
    <p class="note" style="margin-bottom:16px;">
      Charge un fichier CSV contenant les liens et données des cabinets.
      Les colonnes <strong>note_google</strong>, <strong>classement</strong>, <strong>nb_avis</strong> etc. enrichissent l'audit automatiquement.
      <br><br>
    ...
    <div class="lang-selector" style="display:flex;gap:8px;margin-bottom:12px;">
  ...
  <div id="panel-urls" class="panel">
    <p class="note" style="margin-bottom:16px;">
      Entrez les URLs des cabinets. Ajoutez optionnellement la note Google, le nombre d'avis et le classement pour enrichir l'audit.
      Pour plusieurs cabinets, un ZIP est généré.
    </p>
    <div class="url-list" id="urlList">
      <div class="url-card">
        ...
        <div class="url-extras">
          ...
          <div class="field"><label>Classement</label><input type="text" class="class-input" placeholder="Legal 500, Chambers..."></div>
        </div>
      </div>
    </div>
...
<script>
  ...
  // ── Sample CSV download ──────────────────────────────────────────────────────
  const SAMPLE_CSV = ${JSON.stringify(SAMPLE_CSV)};
  ...
  const entries = cards.map(card => {
    ...
    return {
      ...
      overrides: {
        ...
        Nb_Avis_Google: isNaN(avis) ? undefined : avis,
        Nom_Classement: classement || undefined,
      }
    };
  }).filter(e => e.url);
...
document.getElementById('manualForm').addEventListener('submit', async e => {
  e.preventDefault();
  if (!screenshotB64) {
    setStatus('manualStatus', 'Ajoutez une capture d\\'écran.', 'error'); return; }
    ...
    const payload = {
      ...
      Score_Global: parseInt(v('score')) || 64,
      Ville: v('ville') || undefined, Annee_Creation: parseInt(v('creation')) || undefined,
      Nom_Classement: v('classement') || undefined,
      Note_Google: parseFloat(v('google')) || undefined, Nb_Avis_Google: parseInt(v('avis')) || undefined,
      Taille_Equipe: parseInt(v('equipe')) || undefined,
      Specialite_1: v('spec1') || undefined, Specialite_2: v('spec2') || undefined,
      Est_Template: document.getElementById('template').checked,
      Nom_Template: v('cms') || undefined,
      UI_Obsolete: document.getElementById('ui').checked,
      UI_Commentaire: v('uiCom') || undefined,
      Copywriting_Juridique: document.getElementById('copy').checked,
      Pas_Mobile: document.getElementById('mobile').checked,
      Pas_SSL: document.getElementById('ssl').checked,
      Temps_Chargement: parseFloat(v('chargement')) || undefined,
