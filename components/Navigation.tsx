...
const Navigation: React.FC = () => {
  ...
  const links = [
    { label: t('preambule'), href: '/#preambule' },
    { label: t('approche'), href: '/#approche' },
    { label: t('revue'), href: '/#revue' },
  ];
  ...
  return (
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-700 ${isScrolled ? 'py-4 bg-paper/95 backdrop-blur-md border-b border-ink/5' : 'py-6 bg-transparent'
          }`}
      >
        <div className="max-w-[1920px] mx-auto px-6 md:px-12 lg:px-16 xl:px-24 flex justify-between items-center">
          ...
          {/* 1. LOGO - Left aligned */}
          {/* 1. LOGO - Left aligned */}
          <a href="/#hero" className="z-50 flex items-center gap-4 pl-0 xl:pl-8 group cursor-pointer">
            <img
          ...
          <div className="hidden lg:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
            <div className="flex space-x-12 items-center">
              {links.map((link) => (
                ...
                >
                  {link.label}
                  <span className="absolute -bottom-2 left-1/2 w-0 h-px bg-ink velours-transition group-hover:w-full group-hover:left-0"></span>
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-10 pr-0 xl:pr-8">
            ...
            >
              ...
              <ArrowRight className="relative z-10 w-3 h-3 text-ink transform group-hover:translate-x-1 transition-transform duration-500 ease-luxury" />

              {/* Fill Effect - Brass (Laiton) */}
              <div className="absolute inset-0 bg-brass transform scale-x-0 origin-left group-hover:scale-x-100 transition-transform duration-700 ease-luxury z-0"></div>
            </a>
          </div>
